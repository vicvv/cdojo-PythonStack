...
def values_greater_than_second(mlist):
    if len(mlist) < 2:
        return False
    # for i in range(len(mlist)):
    #     print("mlist[i-1]", mlist[i-1], "mlist[i]", mlist[i])
    #     if mlist[i-1]< mlist[i]:
    #         print(mlist[i])

    else:
        return list(mlist[i] for i in range(len(mlist)) if mlist[i-1]< mlist[i])

print(values_greater_than_second([5,2,3,2,1,4]))
mlist = [5,2,3,2,1,4]

#print(list(mlist[i] for i in range(len(mlist)) if mlist[i-1]< mlist[i]))
...
def length_and_value(x,y):
    return list(y for i in range(x))
